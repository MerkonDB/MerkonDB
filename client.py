...
Usage: batch insert <collection_name> <key1>=<value1> <key2>=<value2> ..."""
        args = arg.split(maxsplit=2)
        if len(args) < 2 or args[0] != "insert":
    ...
    def do_generate(self, arg):
        """Generate a proof for a record
Usage: generate proof <collection_name> <key>"""
        args = arg.split()
        if len(args) != 3 or args[0] != "proof":
            print(f"{Fore.RED}✗ Error: Usage: generate proof <collection_name> <key>{Style.RESET_ALL}")
            return
        if not self.current_db:
            print(f"{Fore.RED}✗ Error: No database selected. Use 'use <db_name>' first.{Style.RESET_ALL}")
            return
    ...
    def do_verify(self, arg):
        """Verify a proof for a record or verify database integrity
Usage: verify proof <collection_name> <key> <value> <proof_json>
       verify integrity"""
        args = arg.split(maxsplit=4)
        if len(args) < 1:
            print(f"{Fore.RED}✗ Error: Usage: verify proof <collection_name> <key> <value> <proof_json> or verify integrity{Style.RESET_ALL}")
            return
        if not self.current_db:
            print(f"{Fore.RED}✗ Error: No database selected. Use 'use <db_name>' first.{Style.RESET_ALL}")
            return
        ...
        if args[0] == "proof":
            if len(args) != 5:
                print(f"{Fore.RED}✗ Error: Usage: verify proof <collection_name> <key> <value> <proof_json>{Style.RESET_ALL}")
                return
            try:
                proof = json.loads(args[4])
            except json.JSONDecodeError:
                print(f"{Fore.RED}✗ Error: Invalid proof JSON{Style.RESET_ALL}")
                return
            ...
            self.send_command("db_verify_integrity", {"db_name": self.current_db})
        else:
            print(f"{Fore.RED}✗ Error: Usage: verify proof <collection_name> <key> <value> <proof_json> or verify integrity{Style.RESET_ALL}")

    def do_root(self, arg):
        """Get the root hash of a collection
Usage: root hash <collection_name>"""
        args = arg.split()
        if len(args) != 2 or args[0] != "hash":
            print(f"{Fore.RED}✗ Error: Usage: root hash <collection_name>{Style.RESET_ALL}")
            return
        if not self.current_db:
            print(f"{Fore.RED}✗ Error: No database selected. Use 'use <db_name>' first.{Style.RESET_ALL}")
            return
    ...
    def do_save(self, arg):
        """Save the current database or all databases to disk
Usage: save
       save all"""
        args = arg.split()
        if len(args) == 0:
            ...
            print(f"{Fore.RED}✗ Error: Usage: save or save all{Style.RESET_ALL}")

    def do_load(self, arg):
        """Load a database or all databases from disk
Usage: load <db_name>
       load all"""
        args = arg.split()
        if len(args) == 1:
            if args[0] == "all":
                ...
                self.send_command("db_load", {"db_name": args[0]})
        else:
            print(f"{Fore.RED}✗ Error: Usage: load <db_name> or load all{Style.RESET_ALL}")
...
Usage: compact"""
        if not self.current_db:
            print(f"{Fore.RED}✗ Error: No database selected. Use 'use <db_name>' first.{Style.RESET_ALL}")
            return
        self.send_command("db_compact", {"db_name": self.current_db})
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print(f"{Fore.RED}✗ Usage: python3 client.py <host> <port>{Style.RESET_ALL}")
        sys.exit(1)
    client = MerkonDBClient(sys.argv[1], int(sys.argv[2]))
    client.cmdloop()